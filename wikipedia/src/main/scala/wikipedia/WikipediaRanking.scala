...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate[Int](0)( (value: Int, wa: WikipediaArticle) => { value + (if (wa.text.split(" ").contains(lang)) 1 else 0)} , (a, b) => a + b)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(a => a.text.split(" ").distinct.filter(w => langs.contains(w)).map(w => (w, a)))
       .groupByKey
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(a => a.text.split(" ").distinct.filter(w => langs.contains(w)).map(w => (w, 1)))
      .reduceByKey { case (a:Int, b:Int) => a + b}
