...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType) :: columnNames.tail.map(StructField(_, DoubleType)))
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = expr("CASE WHEN (1 <= telfs AND telfs < 3) THEN 'working' ELSE 'not working' END")
      .as("working")
    val sexProjection: Column = expr("CASE WHEN tesex = 1 THEN 'male' ELSE 'female' END")
      .as("sex")
    val ageProjection: Column = expr("CASE WHEN (15 <= teage AND teage <= 22) THEN 'young' ELSE (CASE WHEN (23 <= teage AND teage <= 55) THEN 'active' ELSE 'elder' END) END")
      .as("age")

    val primaryNeedsProjection: Column = expr(s"(${primaryNeedsColumns.mkString(" + ")}) / 60")
      .as("primaryNeeds")
    val workProjection: Column = expr(s"(${workColumns.mkString(" + ")}) / 60")
      .as("work")
    val otherProjection: Column = expr(s"(${otherColumns.mkString(" + ")}) / 60")
      .as("other")
    df
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(t => (t.working, t.sex, t.age))
          .agg(typed.avg(t => t.primaryNeeds), typed.avg(t => t.work), typed.avg(t => t.other))
          .map(t => TimeUsageRow(t._1._1, t._1._2, t._1._3, round1(t._2), round1(t._3), round1(t._4)))
          .orderBy("working", "sex", "age")
  }
}
...
/**
  ...
  * @param work Number of daily hours spent on other activities
  */
